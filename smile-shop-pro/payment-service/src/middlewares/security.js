...
// Payment-specific rate limiting (stricter for payment operations)
...
// Security headers configuration (PCI compliant)
...
const sanitizeObject = (obj) => {
  for (let key in obj) {
    if (typeof obj[key] === 'string') {
      ...
      // Limit string length for security (payment data can be large)
...
const validateRequest = (req, res, next) => {
  ...
  const suspiciousPatterns = [
    ...
    /(\btoken|secret|key|password)\s*[:=]\s*['"][^'"]{50,}['"]/i, // Long tokens in strings
  ];
...
const requestSizeLimit = (req, res, next) => {
  // Limit request body size (2MB for payment data with metadata, 1MB for regular requests)
...
// Webhook signature verification middleware (for Stripe webhooks)
const verifyWebhookSignature = (req, res, next) => {
  ...
  if (!signature) {
    return res.status(400).json({
      ...
      message: 'Stripe signature is required for webhook verification.',
    });
  }

  // Store signature for later verification in webhook handler
